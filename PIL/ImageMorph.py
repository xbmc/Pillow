...
class LutBuilder(object):
    ...
    def _pattern_permute(self, basic_pattern, options, basic_result):
        ...
        if '4' in options:
            res = patterns[-1][1]
            for i in range(4):
                patterns.append((self._string_permute(patterns[-1][
                    0], [6, 3, 0, 7, 4, 1, 8, 5, 2]), res))
        # mirror
        if 'M' in options:
            n = len(patterns)
            for pattern, res in patterns[0:n]:
                patterns.append((self._string_permute(
                    pattern, [2, 1, 0, 5, 4, 3, 8, 7, 6]), res))
        ...
        if 'N' in options:
            ...
            for pattern, res in patterns[0:n]:
                # Swap 0 and 1
                pattern = (pattern.replace('0', 'Z').replace('1', '0')
                           .replace('Z', '1'))
                res = '%d' % (1 - int(res))
