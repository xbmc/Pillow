...
static int PyImagingPhotoPut(ClientData clientdata, Tcl_Interp *interp,
  ...
  if (argc != 3) {
    Tcl_AppendResult(interp, "usage: ", argv[0], " destPhoto srcImage",
                     (char *)NULL);
    return TCL_ERROR;
  }
...
static int PyImagingPhotoGet(ClientData clientdata, Tcl_Interp *interp,
  ...
  if (argc != 2) {
    Tcl_AppendResult(interp, "usage: ", argv[0], " srcPhoto", (char *)NULL);
    return TCL_ERROR;
  }
  ...
  photo = Tk_FindPhoto(interp, argv[1]);
  if (photo == NULL) {
    Tcl_AppendResult(interp, "source photo must exist", (char *)NULL);
    return TCL_ERROR;
  }
  ...
  Tcl_AppendResult(interp, "this function is not yet supported", (char *)NULL);

  return TCL_ERROR;
}

void TkImaging_Init(Tcl_Interp *interp) {
  Tcl_CreateCommand(interp, "PyImagingPhoto", PyImagingPhotoPut, (ClientData)0,
