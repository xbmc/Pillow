...
static void *getlist(PyObject *arg, int *length, const char *wrong_length,
                     int type) {
  int i, n, itemp;
  double dtemp;
  void *list;
...
static PyObject *_new_array(PyObject *self, PyObject *args) {
  char *mode;
  int xsize, ysize;
...
static PyObject *_getcount(PyObject *self, PyObject *args) {
...
static PyObject *_open_ppm(PyObject *self, PyObject *args) {
  char *filename;
...
static PyObject *_copy2(ImagingObject *self, PyObject *args) {
  ImagingObject *imagep1;
  ImagingObject *imagep2;
  if (!PyArg_ParseTuple(args, "O!O!", &Imaging_Type, &imagep1, &Imaging_Type,
                        &imagep2))
...
static PyObject *_paste(ImagingObject *self, PyObject *args) {
  ...
  if (PyImaging_Check(source))
    ...
    if (!getink(source, self->image, ink))
      return NULL;
    status = ImagingFill2(self->image, ink, (maskp) ? maskp->image : NULL, x0,
                          y0, x1, y1);
  }
    ...
    if (PyBytes_Check(data)) {
      ...
      if (scale == 1.0 && offset == 0.0) {
        ...
        for (i = x = y = 0; i < n; i++) {
          ...
          image->image8[y][x] =
              CLIP((int)(PyFloat_AsDouble(op) * scale + offset));
          if (++x >= (int)image->xsize) {
            x = 0, y++;
          }
        }
...
static PyObject *_putpalette(ImagingObject *self, PyObject *args) {
  ...
  UINT8 *palette;
  int palettesize;
  if (!PyArg_ParseTuple(args, "s" PY_ARG_BYTES_LENGTH, &rawmode, &palette,
                        &palettesize))
    return NULL;
...
static PyObject *_resize(ImagingObject *self, PyObject *args) {
  ...
  if (imIn->xsize == xsize && imIn->ysize == ysize) {
    ...
    imOut =
        ImagingTransformAffine(imOut, imIn, 0, 0, xsize, ysize, a, filter, 1);
  } else {
...
static PyObject *_rotate(ImagingObject *self, PyObject *args) {
  Imaging imOut;
  Imaging imIn;
...
static PyObject *_transform2(ImagingObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "(iiii)O!iO|ii", &x0, &y0, &x1, &y1,
                        &Imaging_Type, &imagep, &method, &data, &filter, &fill))
    return NULL;
  ...
  switch (method) {
  case IMAGING_TRANSFORM_AFFINE:
    imOut = ImagingTransformAffine(imOut, imIn, x0, y0, x1, y1, a, filter, 1);
    break;
  case IMAGING_TRANSFORM_PERSPECTIVE:
    imOut =
        ImagingTransformPerspective(imOut, imIn, x0, y0, x1, y1, a, filter, 1);
    break;
  case IMAGING_TRANSFORM_QUAD:
    imOut = ImagingTransformQuad(imOut, imIn, x0, y0, x1, y1, a, filter, 1);
    break;
  default:
...
static PyObject *_getprojection(ImagingObject *self, PyObject *args) {
  ...
  result = Py_BuildValue(PY_ARG_BYTES_LENGTH PY_ARG_BYTES_LENGTH, xprofile,
                         self->image->xsize, yprofile, self->image->ysize);
...
static PyObject *_font_new(PyObject *self_, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!" PY_ARG_BYTES_LENGTH, &Imaging_Type, &imagep,
...
static PyObject *_draw_chord(ImagingDrawObject *self, PyObject *args) {
  ...
  n = ImagingDrawChord(self->image->image, (int)xy[0], (int)xy[1], (int)xy[2],
                       (int)xy[3], start, end, &ink, fill, self->blend);
...
static PyObject *_draw_line(ImagingDrawObject *self, PyObject *args) {
  int x0, y0, x1, y1;
  int ink;
  if (!PyArg_ParseTuple(args, "(ii)(ii)i", &x0, &y0, &x1, &y1, &ink))
    return NULL;

  if (ImagingDrawLine(self->image->image, x0, y0, x1, y1, &ink, self->blend) <
      0)
    return NULL;
...
static PyObject *_draw_point(ImagingDrawObject *self, PyObject *args) {
  int x, y;
  int ink;
...
static PyObject *_draw_outline(ImagingDrawObject *self, PyObject *args) {
  ...
  if (ImagingDrawOutline(self->image->image, outline, &ink, fill, self->blend) <
      0)
    return NULL;
...
static PyObject *_draw_polygon(ImagingDrawObject *self, PyObject *args) {
  ...
  if (ImagingDrawPolygon(self->image->image, n, ixy, &ink, fill, self->blend) <
      0) {
    free(ixy);
    return NULL;
  }
...
static PyObject *_crc32(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, PY_ARG_BYTES_LENGTH "|(ii)", &buffer, &bytes, &hi,
                        &lo))
    return NULL;
...
static PyObject *_getattr_unsafe_ptrs(ImagingObject *self, void *closure) {
  return Py_BuildValue(
      "(ss)(si)(si)(si)(si)(si)(sn)(sn)(sn)(sn)(sn)(si)(si)(sn)", "mode",
      self->image->mode, "type", self->image->type, "depth", self->image->depth,
      "bands", self->image->bands, "xsize", self->image->xsize, "ysize",
      self->image->ysize, "palette", self->image->palette, "image8",
      self->image->image8, "image32", self->image->image32, "image",
      self->image->image, "block", self->image->block, "pixelsize",
      self->image->pixelsize, "linesize", self->image->linesize, "destroy",
      self->image->destroy);
};
...
static int setup_module(PyObject *m) {
  ...
  PyDict_SetItemString(d, "PILLOW_VERSION",
                       PyUnicode_FromString(PILLOW_VERSION));
...
#else
PyMODINIT_FUNC init_imaging(void) {
