...
typedef struct { PyObject_HEAD cmsHPROFILE profile; } CmsProfileObject;
...
static PyObject *createProfile(PyObject *self, PyObject *args) {
  ...
  if (strcmp(sColorSpace, "LAB") == 0) {
    if (dColorTemp > 0.0) {
      ...
      if (!result) {
        PyErr_SetString(PyExc_ValueError, "ERROR: Could not calculate white "
                                          "point from color temperature "
                                          "provided, must be float in degrees "
                                          "Kelvin");
        return NULL;
      }
...
static PyObject *_xyz3_py(cmsCIEXYZ *XYZ) {
  ...
  return Py_BuildValue("(((d,d,d),(d,d,d),(d,d,d)),((d,d,d),(d,d,d),(d,d,d)))",
                       XYZ[0].X, XYZ[0].Y, XYZ[0].Z, XYZ[1].X, XYZ[1].Y,
                       XYZ[1].Z, XYZ[2].X, XYZ[2].Y, XYZ[2].Z, xyY[0].x,
                       xyY[0].y, xyY[0].Y, xyY[1].x, xyY[1].y, xyY[1].Y,
...
static PyObject *_is_intent_supported(CmsProfileObject *self, int clut) {
  ...
  for (i = 0; i < n; i++) {
    ...
    entry = Py_BuildValue(
        "(OOO)", _check_intent(clut, self->profile, intent, LCMS_USED_AS_INPUT)
                     ? Py_True
                     : Py_False,
...
static PyObject *cms_profile_getattr_product_desc(CmsProfileObject *self,
                                                  void *closure) {
  // description was Description != 'Copyright' || or  "%s - %s" (manufacturer,
  // model) in 1.x
...
cms_profile_getattr_icc_viewing_condition(CmsProfileObject *self,
  ...
  return Py_BuildValue("{s:(ddd),s:(ddd),s:s}", "illuminant",
                       ...
                       vc->SurroundXYZ.Y, vc->SurroundXYZ.Z, "illuminant_type",
...
static PyTypeObject CmsProfile_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "CmsProfile", sizeof(CmsProfileObject), 0,
    /* methods */
...
#else
PyMODINIT_FUNC init_imagingcms(void) {
