...
static PyObject *getfont(PyObject *self_, PyObject *args, PyObject *kw) {
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kw, "eti|iss#", kwlist, Py_FileSystemDefaultEncoding, &filename,
          &size, &index, &encoding, &font_bytes, &font_bytes_size)) {
    return NULL;
  }
...
static PyObject *font_getsize(FontObject *self, PyObject *args) {
  ...
  for (x = i = 0; font_getchar(string, i, &ch); i++) {
    ...
    if (kerning && last_index && index) {
      FT_Vector delta;
      FT_Get_Kerning(self->face, last_index, index, ft_kerning_default, &delta);
      x += delta.x;
    }
  ...
  if (face) {
    ...
    if (xoffset < 0)
      ...
      xoffset = 0;
    /* right bearing */
    offset = face->glyph->metrics.horiAdvance - face->glyph->metrics.width -
  ...
  return Py_BuildValue("(ii)(ii)", PIXEL(x), PIXEL(y_max - y_min),
                       PIXEL(xoffset), yoffset);
}

static PyObject *font_getabc(FontObject *self, PyObject *args) {
  FT_ULong ch;
  FT_Face face;
  double a, b, c;
  ...
  if (font_getchar(string, 0, &ch)) {
    ...
    c = (face->glyph->metrics.horiAdvance - face->glyph->metrics.horiBearingX -
  ...
  for (x = i = 0; font_getchar(string, i, &ch); i++) {
    ...
    if (mask) {
      ...
      for (y = 0; y < glyph->bitmap.rows; y++) {
        int yy =
            y + im->ysize - (PIXEL(glyph->metrics.horiBearingY) + ascender);
        if (yy >= 0 && yy < im->ysize) {
...
#else
PyMODINIT_FUNC init_imagingft(void) {
