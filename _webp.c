...
PyObject *WebPEncode_wrapper(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "s#iiifss#s#", (char **)&rgb, &size, &width,
                        &height, &lossless, &quality_factor, &mode, &icc_bytes,
                        &icc_size, &exif_bytes, &exif_size)) {
    Py_RETURN_NONE;
  }
  if (strcmp(mode, "RGBA") == 0) {
...
#endif
    {
      ret_size = WebPEncodeRGBA(rgb, width, height, 4 * width, quality_factor,
                                &output);
    }
    ...
    {
      ret_size =
          WebPEncodeRGB(rgb, width, height, 3 * width, quality_factor, &output);
    }
  } else {
    Py_RETURN_NONE;
  }
...
#endif
  ret =
      Py_BuildValue("SiiSSS", bytes, config.output.width, config.output.height,
...
static PyMethodDef webpMethods[] = {
    ...
    {"WebPDecoderVersion", WebPDecoderVersion_wrapper, METH_VARARGS,
     "WebPVersion"},
    {"WebPDecoderBuggyAlpha", WebPDecoderBuggyAlpha_wrapper, METH_VARARGS,
