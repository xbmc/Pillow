...
typedef struct { PyObject_HEAD ImagingDIB dib; } ImagingDisplayObject;
...
static LRESULT CALLBACK windowCallback(HWND wnd, UINT message, WPARAM wParam,
  ...
  case WM_SIZE:
    ...
    if (callback) {
      threadstate = (PyThreadState *)GetWindowLongPtr(wnd, sizeof(PyObject *));
...
PyObject *PyImaging_DrawWmf(PyObject *self, PyObject *args) {
  ...
  int width, height;
  int x0, y0, x1, y1;
  if (!PyArg_ParseTuple(args, PY_ARG_BYTES_LENGTH "(ii)(iiii):_load", &data,
                        &datasize, &width, &height, &x0, &x1, &y0, &y1))
    return NULL;
