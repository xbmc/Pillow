...
PyObject *PyImaging_ZipEncoderNew(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "ss|iii" PY_ARG_BYTES_LENGTH, &mode, &rawmode,
                        &optimize, &compress_level, &compress_type, &dictionary,
                        &dictionary_size))
    return NULL;
  ...
  for (i = 0; i < num_tables; i++) {
    ...
    for (j = 0; j < DCTSIZE2; j++) {
      qarrays[i * DCTSIZE2 + j] =
          PyInt_AS_LONG(PySequence_Fast_GET_ITEM(table_data, j));
    }
...
PyObject *PyImaging_JpegEncoderNew(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "ss|iiiiiiiiO" PY_ARG_BYTES_LENGTH
                              "" PY_ARG_BYTES_LENGTH,
                        &mode, &rawmode, &quality, &progressive, &smooth,
                        &optimize, &streamtype, &xdpi, &ydpi, &subsampling,
                        &qtables, &extra, &extra_size, &rawExif, &rawExifLen))
    return NULL;
...
PyObject *PyImaging_LibTiffEncoderNew(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "sssisO", &mode, &rawmode, &compname, &fp,
                        &filename, &dir)) {
    return NULL;
  }
  ...
  for (pos = 0; pos < d_size; pos++) {
    ...
    if (PyInt_Check(value)) {
      ...
      status = ImagingLibTiffSetField(
          &encoder->state, (ttag_t)PyInt_AsLong(key), PyInt_AsLong(value));
    } else if (PyFloat_Check(value)) {
      TRACE(("Setting from Float: %d, %f \n", (int)PyInt_AsLong(key),
             PyFloat_AsDouble(value)));
      status =
          ImagingLibTiffSetField(&encoder->state, (ttag_t)PyInt_AsLong(key),
                                 (float)PyFloat_AsDouble(value));
    } else if (PyBytes_Check(value)) {
      TRACE(("Setting from Bytes: %d, %s \n", (int)PyInt_AsLong(key),
             PyBytes_AsString(value)));
      status = ImagingLibTiffSetField(
          &encoder->state, (ttag_t)PyInt_AsLong(key), PyBytes_AsString(value));
      ...
      if (len) {
        if (PyInt_Check(PyTuple_GetItem(value, 0))) {
          ...
          if (intav) {
            ...
            status = ImagingLibTiffSetField(
                &encoder->state, (ttag_t)PyInt_AsLong(key), len, intav);
            free(intav);
          }
          ...
          if (floatav) {
            ...
            status = ImagingLibTiffSetField(
                &encoder->state, (ttag_t)PyInt_AsLong(key), len, floatav);
            free(floatav);
          }
        } else {
...
PyObject *PyImaging_Jpeg2KEncoderNew(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "ss|OOOsOIOOOssi", &mode, &format, &offset,
