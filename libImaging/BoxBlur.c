  ...
  if (imIn->image8) {
    for (y = 0; y < imIn->ysize; y++) {
      ImagingLineBoxBlur8((imIn == imOut ? (UINT8 *)lineOut : imOut->image8[y]),
                          imIn->image8[y], imIn->xsize - 1, radius, edgeA,
                          edgeB, ww, fw);
      if (imIn == imOut) {
        // Commit.
    ...
    for (y = 0; y < imIn->ysize; y++) {
      ImagingLineBoxBlur32(imIn == imOut ? (pixel *)lineOut
                           ...
                           (pixel *)imIn->image32[y], imIn->xsize - 1, radius,
                           edgeA, edgeB, ww, fw);
      if (imIn == imOut) {
        // Commit.
...
Imaging ImagingBoxBlur(Imaging imOut, Imaging imIn, float radius, int n) {
  ...
  if (!(strcmp(imIn->mode, "RGB") == 0 || strcmp(imIn->mode, "RGBA") == 0 ||
        strcmp(imIn->mode, "RGBX") == 0 || strcmp(imIn->mode, "CMYK") == 0 ||
        strcmp(imIn->mode, "L") == 0 || strcmp(imIn->mode, "LA") == 0))
    return ImagingError_ModeError();
