...
static void rgb2hsv(UINT8 *out, const UINT8 *in,
...
#pragma optimize("", off)
#endif
static Imaging topalette(Imaging imOut, Imaging imIn, ImagingPalette inpalette,
                         int dither) {
  ImagingSectionCookie cookie;
  int x, y;
...
Imaging ImagingConvertTransparent(Imaging imIn, const char *mode, int r, int g,
  ...
  if (!((strcmp(imIn->mode, "RGB") == 0 || strcmp(imIn->mode, "L") == 0) &&
        strcmp(mode, "RGBA") == 0))
#ifdef notdef
  {
    return (Imaging)ImagingError_ValueError("conversion not supported");
