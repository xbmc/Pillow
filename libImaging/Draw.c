...
int ImagingDrawPolygon(Imaging im, int count, int *xy, const void *ink_,
  ...
  if (fill) {
    ...
    /* Outline */
    for (i = 0; i < count - 1; i++)
      draw->line(im, xy[i + i], xy[i + i + 1], xy[i + i + 2], xy[i + i + 3],
                 ink);
...
static int ellipse(Imaging im, int x0, int y0, int x1, int y1, float start,
                   float end, const void *ink_, int fill, int mode, int op) {
  float i;
  int n;
  int cx, cy;
  int w, h;
...
int ImagingDrawArc(Imaging im, int x0, int y0, int x1, int y1, float start,
                   float end, const void *ink, int op) {
...
int ImagingDrawChord(Imaging im, int x0, int y0, int x1, int y1, float start,
                     float end, const void *ink, int fill, int op) {
...
int ImagingOutlineCurve2(ImagingOutline outline, float cx, float cy, float x3,
                         float y3) {
  /* add bezier curve based on three control points (as
     in the Flash file format) */

  return ImagingOutlineCurve(outline, (outline->x + cx + cx) / 3,
                             (outline->y + cy + cy) / 3, (cx + cx + x3) / 3,
                             (cy + cy + y3) / 3, x3, y3);
}
