...
Imaging ImagingExpand(Imaging imIn, int xmargin, int ymargin, int mode) {
  ...
  imOut = ImagingNew(imIn->mode, imIn->xsize + 2 * xmargin,
...
#define KERNEL3x3(image, kernel, d)                                            \
  ((int)image[y + 1][x - d] * kernel[0] + (int)image[y + 1][x] * kernel[1] +   \
   (int)image[y + 1][x + d] * kernel[2] + (int)image[y][x - d] * kernel[3] +   \
   (int)image[y][x] * kernel[4] + (int)image[y][x + d] * kernel[5] +           \
   (int)image[y - 1][x - d] * kernel[6] + (int)image[y - 1][x] * kernel[7] +   \
   (int)image[y - 1][x + d] * kernel[8])

#define KERNEL5x5(image, kernel, d)                                            \
  ((int)image[y + 2][x - d - d] * kernel[0] +                                  \
   (int)image[y + 2][x - d] * kernel[1] + (int)image[y + 2][x] * kernel[2] +   \
   ...
   (int)image[y][x - d - d] * kernel[10] + (int)image[y][x - d] * kernel[11] + \
   (int)image[y][x] * kernel[12] + (int)image[y][x + d] * kernel[13] +         \
   (int)image[y][x + d + d] * kernel[14] +                                     \
   (int)image[y - 1][x - d - d] * kernel[15] +                                 \
   (int)image[y - 1][x - d] * kernel[16] + (int)image[y - 1][x] * kernel[17] + \
   (int)image[y - 1][x + d] * kernel[18] +                                     \
   (int)image[y - 1][x + d + d] * kernel[19] +                                 \
   (int)image[y - 2][x - d - d] * kernel[20] +                                 \
   (int)image[y - 2][x - d] * kernel[21] + (int)image[y - 2][x] * kernel[22] + \
