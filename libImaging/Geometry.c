...
#define ROTATE_90(image)                                                       \
  for (y = 0; y < imIn->ysize; y += ROTATE_CHUNK) {                            \
    for (x = 0; x < imIn->xsize; x += ROTATE_CHUNK) {                          \
      yysize =                                                                 \
          y + ROTATE_CHUNK < imIn->ysize ? y + ROTATE_CHUNK : imIn->ysize;     \
      xxsize =                                                                 \
          x + ROTATE_CHUNK < imIn->xsize ? x + ROTATE_CHUNK : imIn->xsize;     \
...
Imaging ImagingTransposeToNew(Imaging imIn) {
...
static int affine_transform(double *xin, double *yin, int x, int y,
                            void *data) {
...
static int perspective_transform(double *xin, double *yin, int x, int y,
                                 void *data) {
  double *a = (double *)data;
  double a0 = a[0];
  double a1 = a[1];
...
static int quad_transform(double *xin, double *yin, int x, int y, void *data) {
...
Imaging ImagingTransform(Imaging imOut, Imaging imIn, int x0, int y0, int x1,
                         int y1, ImagingTransformMap transform,
                         void *transform_data, ImagingTransformFilter filter,
                         void *filter_data, int fill) {
...
Imaging ImagingTransformAffine(Imaging imOut, Imaging imIn, int x0, int y0,
  ...
  if (filterid || imIn->type == IMAGING_TYPE_SPECIAL) {
    ...
    return ImagingTransform(imOut, imIn, x0, y0, x1, y1, affine_transform, a,
                            filter, NULL, fill);
  }
...
Imaging ImagingTransformPerspective(Imaging imOut, Imaging imIn, int x0, int y0,
                                    int x1, int y1, double a[8], int filterid,
                                    int fill) {
  ImagingTransformFilter filter = getfilter(imIn, filterid);
  if (!filter)
    return (Imaging)ImagingError_ValueError("bad filter number");

  return ImagingTransform(imOut, imIn, x0, y0, x1, y1, perspective_transform, a,
                          filter, NULL, fill);
}

Imaging ImagingTransformQuad(Imaging imOut, Imaging imIn, int x0, int y0,
                             int x1, int y1, double a[8], int filterid,
                             int fill) {
  ImagingTransformFilter filter = getfilter(imIn, filterid);
  if (!filter)
  ...
  return ImagingTransform(imOut, imIn, x0, y0, x1, y1, quad_transform, a,
                          filter, NULL, fill);
}
...
Imaging ImagingRotate(Imaging imOut, Imaging imIn, double theta, int filterid) {
  int xsize, ysize;
  ...
  return ImagingTransformAffine(imOut, imIn, 0, 0, imOut->xsize, imOut->ysize,
                                a, filterid, 1);
}
