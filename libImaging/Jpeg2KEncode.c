...
/* OpenJPEG 2.0 doesn't have OPJ_VERSION_MAJOR */
  ...
  /* Write each tile */
  tiles_x =
      (im->xsize + (params.image_offset_x0 - params.cp_tx0) + tile_width - 1) /
      tile_width;
  tiles_y =
      (im->ysize + (params.image_offset_y0 - params.cp_ty0) + tile_height - 1) /
      tile_height;
...
int ImagingJpeg2KEncode(Imaging im, ImagingCodecState state, UINT8 *buf,
  ...
  if (state->state == J2K_STATE_START) {
    int seekable =
        (context->format != OPJ_CODEC_J2K ? INCREMENTAL_CODEC_SEEKABLE
                                          : INCREMENTAL_CODEC_NOT_SEEKABLE);

    context->encoder = ImagingIncrementalCodecCreate(
        j2k_encode_entry, im, state, INCREMENTAL_CODEC_WRITE, seekable,
        context->fd);
