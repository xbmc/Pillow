...
typedef struct { uint32_t scale; } PixelHashData;
...
static int k_means(Pixel *pixelData, uint32_t nPixels, Pixel *paletteData,
  ...
  avgDistSortKey =
      malloc(sizeof(uint32_t *) * nPaletteEntries * nPaletteEntries);
  if (!avgDistSortKey) {
    goto error_2;
  }
...
int quantize(Pixel *pixelData, uint32_t nPixels, uint32_t nQuantPixels,
  ...
  if (!map_image_pixels_from_median_box(pixelData, nPixels, p, nPaletteEntries,
                                        h, avgDist, avgDistSortKey, qp)) {
    goto error_7;
  }
