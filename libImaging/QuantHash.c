...
static void _hashtable_destroy(const HashTable *h, const HashKey_t key,
                               const HashVal_t val, void *u) {
  if (h->keyDestroyFunc) {
...
static int _hashtable_lookup_or_insert(HashTable *h, HashKey_t key,
                                       HashVal_t *retVal, HashVal_t newVal,
                                       int resize) {
  HashNode **n, *nv;
  HashNode *t;
  int i;
...
static int _hashtable_remove(HashTable *h, const HashKey_t key,
                             HashKey_t *keyRet, HashVal_t *valRet, int resize) {
  uint32_t hash = h->hashFunc(h, key) % h->length;
  HashNode *n, *p;
  int i;
  ...
  for (p = NULL, n = h->table[hash]; n; p = n, n = n->next) {
    ...
    if (!i) {
      if (p)
        ...
        h->table[hash] = n->next;
      if (h->valDestroyFunc) {
        h->valDestroyFunc(h, n->value);
      }
      if (h->keyDestroyFunc) {
        h->keyDestroyFunc(h, n->key);
      }
      free(n);
      h->count++;
      return 1;
...
int hashtable_remove(HashTable *h, const HashKey_t key, HashKey_t *keyRet,
                     HashVal_t *valRet) {
...
void hashtable_rehash(HashTable *h) { _hashtable_rehash(h, NULL, h->length); }
