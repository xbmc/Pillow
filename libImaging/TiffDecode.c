...
int ImagingLibTiffDecode(Imaging im, ImagingCodecState state, UINT8 *buffer,
  ...
  // with shuffle.  (or, just alloc a buffer myself, then figure out how to get
  // it
  ...
  while (state->y < state->ysize) {
    if (TIFFReadScanline(tiff, (tdata_t)state->buffer, (uint32)state->y, 0) ==
        -1) {
...
int ImagingLibTiffEncode(Imaging im, ImagingCodecState state, UINT8 *buffer,
  ...
  */
  ...
  if (state->state == 0) {
    ...
    while (state->y < state->ysize) {
      state->shuffle(state->buffer, (UINT8 *)im->image[state->y + state->yoff] +
                                        state->xoff * im->pixelsize,
                     state->xsize);
