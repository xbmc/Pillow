...
int PyPath_Flatten(PyObject *data, double **pxy) {
  int i, j, n;
  double *xy;
...
static PyObject *path_clip_polygon(PyPathObject *self, PyObject *args) {
  /* Clip path representing a single polygon */
  PyErr_SetString(PyExc_RuntimeError, "not yet implemented");
  return NULL;
}

static PyObject *path_clip_polyline(PyPathObject *self, PyObject *args) {
...
static PyObject *path_getitem(PyPathObject *self, int i) {
  if (i < 0)
    i = self->count + i;
  if (i < 0 || i >= self->count) {
...
static int path_setitem(PyPathObject *self, int i, PyObject *op) {
  double *xy;
...
static PyObject *path_subscript(PyPathObject *self, PyObject *item) {
  ...
  if (PySlice_Check(item)) {
    ...
    if (PySlice_GetIndicesEx(item, len, &start, &stop, &step, &slicelength) < 0)
      return NULL;
#else
    if (PySlice_GetIndicesEx((PySliceObject *)item, len, &start, &stop, &step,
                             &slicelength) < 0)
      return NULL;
#endif
